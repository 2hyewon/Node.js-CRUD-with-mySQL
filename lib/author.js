...
exports.list = function(response){
  db.query('SELECT * FROM topic', function(error, topics){
      ...
      db.query('SELECT * FROM author', function(error2, authors){
        ...
        var html = template.HTML('', '', '', listTopics,
...
exports.search_result = function(request, response, queryName){
  db.query(`SELECT * FROM topic LEFT JOIN author ON topic.author_id=author.id where author.name LIKE '${queryName}'`, function(error, results){
    if(error){
      throw error;
    }
    var title = `search - ${queryName}`;
    var search = `<form action="/search_result" method="get">
    <p><input type="text" name="title" placeholder="title">
    <input type="submit" value="search"></p></form>`
    var list = template.listForAuthorSearchResult(results, queryName);
    var html = template.HTML('', search, title, '', list, '');
    response.writeHead(200);
    response.end(html);
  });
}
