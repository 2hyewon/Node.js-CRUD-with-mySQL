...
module.exports = {
  HTML:function(title, search, list, author, control, body){
    ...
    var titlePage = `page=${queryPage}&id=${results[i].id}`;
    if(querySort != '' && querySort !== undefined){
      titlePage += `&sort=${querySort}`;
    }
    while(i < results.length){
      list += `
      <tr>
      <td>${rowNum}</td>
      <td><a href="/?${titlePage}">${results[i].title}</a></td>
  ...
  while(i < authors.length){
    if(authors[i].id == topic[0].author_id){
      tag += `<option value="${authors[i].name}" selected>${authors[i].name}</option>`;
    } else{
      tag += `<option value="${authors[i].name}">${authors[i].name}</option>`;}
      i++;
    }
    return `<select name="authorSelect">${tag}</select>`;
